...
> 把代码工程的模块文档，编译成带有显式依赖关系的结构化知识图谱。

LLM 在文档生成阶段读取代码，在每篇文档的 `Related Concepts` 章节里显式声明依赖关系。这些声明是知识图谱的边，让 AI 工作时可以从任意模块出发，沿依赖链精确收集完整上下文，而不是靠向量相似度猜测。

> 背景与方案选型详见 [项目立项书](../docs/proposal.md)。
